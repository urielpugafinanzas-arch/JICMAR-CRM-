...
const Pedidos = {
  async crearNuevo(datos) {
    try {
      if (!datos.clienteId && datos.tipoVenta !== 'muestra') return { success: false, error: 'Se requiere un cliente' };
  ...
  async registrarPago(pedidoId, pago) {
    try {
      ...
      await addDoc(subRef, {
        ...
        detalleVendido: pago.detalleVendido || null,
        detalleDevuelto: pago.detalleDevuelto || null,
        origen: pago.origen || 'legado',
        notas: pago.notas || '',
  ...
  async eliminarPago(pedidoId, pagoId) {
    try {
      await deleteDoc(doc(db, COLLECTIONS.PEDIDOS, pedidoId, 'pagos', pagoId));
      return { success: true };
    } catch (error) {
      return { success: false, error: error.message };
    }
  ...
  async recalcularDesdePagos(pedidoId) {
    try {
      ...
      const q = query(subRef, orderBy('fecha', 'asc'));
      const snapshot = await getDocs(q);
      const pagos = snapshot.docs.map(d => d.data());
      ...
      pagos.forEach(pago => {
        montoCobrado += pago.monto || 0; // esto siempre fue "lo pagado ese día", nunca acumulado

        if (pago.origen === 'delta') {
          // Pago nuevo: guarda solo lo NUEVO de ese día -> se SUMA
          montoDevuelto += pago.montoDevuelto || 0;
          unidadesDevueltas += pago.unidadesDevueltas || 0;
          (pago.detalleVendido || []).forEach(d => {
            mapaVendido[d.producto] = (mapaVendido[d.producto] || 0) + d.cantidad;
          });
          (pago.detalleDevuelto || []).forEach(d => {
            mapaDevuelto[d.producto] = (mapaDevuelto[d.producto] || 0) + d.cantidad;
          });
        } else {
          // Pago viejo (legado): guardaba el ACUMULADO total hasta ese momento -> REEMPLAZA, no se suma
          montoDevuelto = pago.montoDevuelto || 0;
          unidadesDevueltas = pago.unidadesDevueltas || 0;
          (pago.detalleVendido || []).forEach(d => {
            mapaVendido[d.producto] = d.cantidad;
          });
          (pago.detalleDevuelto || []).forEach(d => {
            mapaDevuelto[d.producto] = d.cantidad;
          });
        }
      });

      const detalleVendido = Object.entries(mapaVendido).map(([producto, cantidad]) => ({ producto, cantidad }));
      const detalleDevuelto = Object.entries(mapaDevuelto).map(([producto, cantidad]) => ({ producto, cantidad }));
      const nuevoSaldo = totalEntregado - montoDevuelto - montoCobrado;

      const productos = pedido.productosEntregados || pedido.productos || [];
      const todoResuelto = productos.length > 0 && productos.every(pr => {
        const cantEntregada = pr.cantidadEntregada ?? pr.cantidad ?? 0;
        const vend = mapaVendido[pr.producto] || 0;
        const dev = mapaDevuelto[pr.producto] || 0;
      ...
      return { success: true, montoCobrado, deuda: nuevoSaldo > 0.01 ? nuevoSaldo : 0 };
    } catch (error) {
      return { success: false, error: error.message };
    }
  },
  ...
  try {
    const q = query(
      ...
      where('fechaEntregadoReal', '>=', inicio),
      where('fechaEntregadoReal', '<=', fin),
      orderBy('fechaEntregadoReal', 'desc')
    );
    const snapshot = await getDocs(q);
...
const Inventario = {
  async obtener() {
    try {
      ...
      const snap = await getDoc(docRef);
      if (snap.exists()) return { success: true, stock: snap.data().stock || {} };
      return { success: true, stock: {} };
    } catch (error) {
      return { success: false, stock: {}, error: error.message };
    }
  },
  ...
  async guardar(stock) {
    try {
      const docRef = doc(db, 'config', 'inventario');
      await setDoc(docRef, { stock, updatedAt: serverTimestamp() });
      return { success: true };
    } catch (error) {
      return { success: false, error: error.message };
    }
  ...
  async registrarMovimiento(tipo, productos, referencia) {
    try {
      const subRef = collection(db, 'inventario_movimientos');
      await addDoc(subRef, {
        tipo,
        productos,
  ...
  async obtenerMovimientos() {
    try {
      const q = query(collection(db, 'inventario_movimientos'), orderBy('fecha', 'desc'));
      const snap = await getDocs(q);
      return { success: true, movimientos: snap.docs.map(d => ({ id: d.id, ...d.data() })) };
    } catch (error) {
      return { success: false, movimientos: [], error: error.message };
    }
  },
  ...
  escuchar(callback) {
    ...
    return onSnapshot(docRef, (snap) => {
      if (snap.exists()) callback(snap.data().stock || {});
      else callback({});
    });
  }
};
