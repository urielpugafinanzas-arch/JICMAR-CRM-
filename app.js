...
window.confirmarCobro = async function () {
  const pedido = App.pedidosList.find(p => p.id === pedidoEnModal);
  if (!pedido) return;// NUEVO: bloquear si ya está liquidada
...
// Vendido = venta directa + consigna LIQUIDADA (pagado/parcial)
// Consigna activa (entregado sin cobrar) = sección separada, NO suma
function generarSeccionVentas(pedidos) {

  const fmt = v => v.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' });

  // ── 1. Pedidos que SÍ cuentan como "vendido" ──────────────────
  //    - Venta directa: pagado o parcial
  //    - Consignación: solo pagado o parcial (ya se liquidó total o parcialmente)
  const vendidos = pedidos.filter(p =>
    ['pagado', 'parcial'].includes(p.estado)
  );

  // ── 2. Consignas ACTIVAS (en calle, aún sin cobrar) ───────────
  const consignasActivas = pedidos.filter(p =>
    p.tipoVenta === 'consignacion' && p.estado === 'entregado'
  );

  // ── 3. Construir mapa de productos vendidos ───────────────────
  const mapa = {};
  ...
  // Restar devoluciones del total monetario
  ...
  // Pendiente de cobrar en consignas activas
  ...
  // ── 6. Sección separada: Consignas activas en calle ──────────
  html += `
...
window.editarCobro = function (id) {
  ...
  const montoInput = document.getElementById('modal-cobro-monto');
  if (montoInput) montoInput.value = yaCobrado; // pre-llena con lo ya cobrado
  const notasInput = document.getElementById('modal-cobro-notas');
  if (notasInput) notasInput.value = pedido.notasCobro || '';

  // Mostrar sección devoluciones en edición
  const secDev = document.getElementById('seccion-devoluciones');
  if (secDev) secDev.style.display = 'block';

  // Pre-llenar devoluciones si ya existían
  const devInput = document.getElementById('modal-cobro-devueltas');
  ...
  reader.onload = async (ev) => {
    try {
      ...
      for (const p of datos.pedidos) {
        const { id, ...resto } = p;
        const r = await Pedidos.actualizar(id, resto).catch(() => null);
        if (r?.success) okPedidos++;
      }
      for (const c of datos.clientes) {
        const { id, ...resto } = c;
        const r = await Clientes.actualizar(id, resto).catch(() => null);
