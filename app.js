...
window.calcularSaldoCobro = function () {
  ...
  // Calcular monto devuelto sumando producto por producto
  ...
  productos.forEach((pr, i) => {
    const inputCant = document.getElementById(`cobro-dev-cant-${i}`);
    if (!inputCant) return;
    const cantDev = parseFloat(inputCant.value) || 0;
    const precio = pr.precioUnitario || 0;
    const subtotal = cantDev * precio;
    montoDevuelto += subtotal;
    if (cantDev > 0) {
      detalleDevolucion += `↩️ ${pr.producto}: ${cantDev} × $${precio} = ${subtotal.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}  `;
    }

    const cantEntregada = pr.cantidadEntregada ?? pr.cantidad ?? 0;
    const inputVend = document.getElementById(`cobro-vend-cant-${i}`);
    const cantVend = parseFloat(inputVend?.value) || 0;
    if (cantVend > 0) {
      const subtotalVend = cantVend * precio;
      montoVendido += subtotalVend;
      detalleVenta += `✅ ${pr.producto}: ${cantVend} × $${precio} = ${subtotalVend.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}  `;
    }
    const pendienteEl = document.getElementById(`cobro-sindefinir-${i}`);
    if (pendienteEl) {
      const pendiente = cantEntregada - cantDev - cantVend;
      pendienteEl.textContent = pendiente;
      pendienteEl.style.color = pendiente < 0 ? '#f87171' : pendiente === 0 ? '#4ade80' : 'inherit';
    }
  });
  ...
  if (devInfo) {
    if (montoDevuelto > 0 || montoVendido > 0) {
      let texto = '';
      if (detalleVenta) texto += detalleVenta;
      if (detalleDevolucion) texto += detalleDevolucion;
      texto += `Total a cobrar: ${montoVendido.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}`;
...
window.confirmarCobro = async function () {
  ...
  let montoDevueltoCheck = 0;
  productosCheck.forEach((pr, i) => {
    const inputCant = document.getElementById(`cobro-dev-cant-${i}`);
    const cantDev = parseFloat(inputCant?.value) || 0;
    montoDevueltoCheck += cantDev * (pr.precioUnitario || 0);
  });
  ...
  productos.forEach((pr, i) => {
    const inputCant = document.getElementById(`cobro-dev-cant-${i}`);
    const cantDev = parseFloat(inputCant?.value) || 0;
    montoDevuelto += cantDev * (pr.precioUnitario || 0);
    unidadesDevueltas += cantDev;
  });
  const totalRealDescontado = totalEntregado - montoDevuelto;
  const nuevoSaldo = totalRealDescontado - nuevoTotalCobrado;

const detalleDevuelto = productos.map((pr, i) => {
    const inputCant = document.getElementById(`cobro-dev-cant-${i}`);
    const cantDev = parseFloat(inputCant?.value) || 0;
    return cantDev > 0 ? { producto: pr.producto, cantidad: cantDev } : null;
  }).filter(Boolean);

const detalleVendido = productos.map((pr, i) => {
    const inputCant = document.getElementById(`cobro-vend-cant-${i}`);
    const cantVend = parseFloat(inputCant?.value) || 0;
    return cantVend > 0 ? { producto: pr.producto, cantidad: cantVend } : null;
  }).filter(Boolean);
  ...
  const datos = {
    ...
    detalleDevuelto: detalleDevuelto.length > 0 ? detalleDevuelto : null,
    detalleVendido: detalleVendido.length > 0 ? detalleVendido : null,
    ...(nuevoSaldo <= 0.01 ? { fechaCierre: fechaLocalHoy() } : {}),
  };
  ...
  try {
    ...
    if (!modoEdicionActual && (montoPagando > 0 || unidadesDevueltas > 0 || detalleVendido.length > 0)) {
    ...
    if (unidadesDevueltas > 0) {
      const stockActual = { ...App.inventarioStock };
      const productos = pedido.productosEntregados || pedido.productos || [];
      const productosConDevolucion = productos.map((pr, i) => {
        const inputCant = document.getElementById(`cobro-dev-cant-${i}`);
        const cantDev = parseFloat(inputCant?.value) || 0;
        return { producto: pr.producto, cantidadDevuelta: cantDev };
      });

      const movimientos = expandirProductosParaInventario(productosConDevolucion, pr => pr.cantidadDevuelta);
      movimientos.forEach(m => {
        stockActual[m.producto] = (stockActual[m.producto] || 0) + m.cantidad;
      });

      await Inventario.guardar(stockActual);
      await Inventario.registrarMovimiento('devolucion', movimientos,
