...
function renderCatalogo() {
  ...
  container.innerHTML = PRODUCTOS_CATALOGO.map((item, idx) => {
    ...
    <div style="display:flex;align-items:center;gap:0.6rem;padding:0.65rem 0.75rem;
      ...
      <div id="cat-inputs-${idx}" style="display:none;flex:1;gap:0.4rem;">
...
window.editarProductoCatalogo = function (idx) {
  ...
  document.getElementById(`cat-input-nombre-${idx}`).focus();
};
...
function generarSeccionConsignas(pedidos) {
  ...
  let html = `<div class="seccion">
    ...
    <table><thead><tr>
      ...
      <th class="col-num">Pendiente</th><th>Fecha cierre</th><th>Estado</th>
    </tr></thead><tbody>`;
  ...
  enCalle.forEach(p => {
    ...
    html += `<tr>
      ...
      <td style="font-size:10px;color:#555;">${p.fechaCierre || '—'}</td>
...
function renderInventario() {
  ...
  container.innerHTML = productos.map(item => {
    const nombre = typeof item === 'string' ? item : item.nombre;
    const cantidad = stock[nombre] || 0;
    const color = cantidad <= 0 ? '#f87171' : cantidad <= 5 ? '#fbbf24' : '#4ade80';
    const icono = cantidad <= 0 ? '🔴' : cantidad <= 5 ? '🟡' : '🟢';
    return `
      <div style="display:flex;align-items:center;justify-content:space-between;
        padding:0.75rem 1rem;background:var(--surface2);border-radius:10px;
        margin-bottom:0.5rem;border:1px solid var(--border);">
        <span style="font-size:0.88rem;flex:1;">${icono} ${nombre}</span>
        <span style="font-size:1rem;font-weight:700;color:${color};min-width:3rem;text-align:right;">${cantidad}</span>
      </div>`;
  }).join('');
}
