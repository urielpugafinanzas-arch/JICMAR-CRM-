...
window.calcularSaldoCobro = function () {
  ...
  if (devInfo) {
    if (montoDevuelto > 0) {
      devInfo.textContent = `${detalleDevolucion}Total descuento: -${montoDevuelto.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })} → Cobrar: ${totalRealACobrar.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}`;
    } else {
      devInfo.textContent = '';
    }
  }
