...
window.confirmarEntrega = async function () {
  ...
  try {
    ...
    const stockActual = { ...App.inventarioStock };
    const sinStock = productosEntregados.filter(pr => {
      const disponible = stockActual[pr.producto] || 0;
      return pr.cantidadEntregada > 0 && disponible < pr.cantidadEntregada;
    });
    if (sinStock.length > 0) {
      const nombres = sinStock.map(pr => `${pr.producto} (disponible: ${stockActual[pr.producto] || 0})`).join(', ');
  ...
  try {
    ...
    if (unidadesDevueltas > 0) {
      ...
      productos.forEach((pr, i) => {
        const inputCant = document.getElementById(`cobro-dev-cant-${i}`);
        const cantDev = parseFloat(inputCant?.value) || 0;
        if (cantDev <= 0 || !pr.producto) return;
        stockActual[pr.producto] = (stockActual[pr.producto] || 0) + cantDev;
        movimientos.push({ producto: pr.producto, cantidad: cantDev });
      });
