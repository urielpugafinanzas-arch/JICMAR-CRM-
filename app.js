...
// ============================================================
// CONFIGURACIÓN DE PRECIOS POR MAYOREO (Fase 1)
// ============================================================
const PRECIOS_MAYOREO = {
  'Maíz Salsas negras': { normal: 12.74, mayoreo1: 12.00, mayoreo2: 11.25, mayoreo3: 10.50 },
  'Maíz Sal y limón':   { normal: 12.74, mayoreo1: 12.00, mayoreo2: 11.25, mayoreo3: 10.50 },
  'Maíz Flaming hot':   { normal: 12.74, mayoreo1: 12.00, mayoreo2: 11.25, mayoreo3: 10.50 },
  'Maíz Jalapeño':      { normal: 12.74, mayoreo1: 12.00, mayoreo2: 11.25, mayoreo3: 10.50 },

  'Chicharrón Salsas negras': { normal: 22.52, mayoreo1: 21.77, mayoreo2: 21.02, mayoreo3: 20.27 },
  'Chicharrón Sal y limón':   { normal: 22.52, mayoreo1: 21.77, mayoreo2: 21.02, mayoreo3: 20.27 },
  'Chicharrón Flaming hot':   { normal: 22.52, mayoreo1: 21.77, mayoreo2: 21.02, mayoreo3: 20.27 },

  'Paquete Maíz Mix':        { normal: 12.74, mayoreo1: 12.00, mayoreo2: 11.25, mayoreo3: 10.50 },
  'Paquete Chicharrón Mix':  { normal: 22.52, mayoreo1: 21.77, mayoreo2: 21.02, mayoreo3: 20.27 },
};

function piezasPorPaqueteDe(nombreProducto) {
  if (nombreProducto.includes('Maíz')) return 8;
  if (nombreProducto.includes('Chicharrón')) return 6;
  return 12;
}

// ============================================================
// TABLA DE PAQUETES — qué contiene cada paquete y qué descuenta
// ============================================================
// ============================================================
// PAQUETES MIX — usan el precio de cualquier sabor de su categoría
// ============================================================
const PAQUETES_MIX_REFERENCIA = {
  'Paquete Maíz Mix': 'Maíz Jalapeño',
  'Paquete Chicharrón Mix': 'Chicharrón Sal y limón',
};

function esPaqueteMix(nombreProducto) {
  return Object.prototype.hasOwnProperty.call(PAQUETES_MIX_REFERENCIA, nombreProducto);
}

function esPaquete(nombreProducto) {
  return Object.prototype.hasOwnProperty.call(PRECIOS_MAYOREO, nombreProducto) && nombreProducto.startsWith('Paquete ');
}

function catalogoSinPaquetes() {
  return PRODUCTOS_CATALOGO.filter(p => {
    const nombre = typeof p === 'string' ? p : p.nombre;
    return !esPaquete(nombre);
  });
}
function expandirProductoParaInventario(nombreProducto, cantidadPiezas) {
  if (esPaqueteMix(nombreProducto)) {
    const paquete = PAQUETES[nombreProducto];
    const piezasPorEstePaquete = paquete
      ? Object.values(paquete.contenido).reduce((s, c) => s + c, 0)
      : piezasPorPaqueteDe(nombreProducto);
    const numPaquetes = cantidadPiezas / piezasPorEstePaquete;
    const desglose = desglosePiezasDePaquete(nombreProducto, numPaquetes);
    return Object.entries(desglose).map(([producto, cantidad]) => ({ producto, cantidad }));
  }
  return [{ producto: nombreProducto, cantidad: cantidadPiezas }];
}
function expandirProductosParaInventario(productos, obtenerCantidad) {
  const resultado = [];
  productos.forEach(pr => {
    const cantidad = obtenerCantidad(pr);
    if (!pr.producto || cantidad <= 0) return;
    const expandido = expandirProductoParaInventario(pr.producto, cantidad);
    expandido.forEach(e => {
      const existente = resultado.find(r => r.producto === e.producto);
      if (existente) existente.cantidad += e.cantidad;
...
const PAQUETES = {
  'Paquete Maíz Jalapeño': {
    categoria: 'Maíz',
    contenido: { 'Maíz Jalapeño': 12 }
  },
  'Paquete Maíz Sal y limón': {
    categoria: 'Maíz',
    contenido: { 'Maíz Sal y limón': 12 }
  },
  'Paquete Maíz Salsas negras': {
    categoria: 'Maíz',
    contenido: { 'Maíz Salsas negras': 12 }
  },
  'Paquete Maíz Flaming hot': {
    categoria: 'Maíz',
    contenido: { 'Maíz Flaming hot': 12 }
  },
  'Paquete Maíz Mix': {
    categoria: 'Maíz',
    contenido: {
      'Maíz Jalapeño': 2,
      'Maíz Sal y limón': 2,
      'Maíz Salsas negras': 2,
      'Maíz Flaming hot': 2
    }
  },
  'Paquete Chicharrón Sal y limón': {
    categoria: 'Chicharrón',
    contenido: { 'Chicharrón Sal y limón': 12 }
  },
  'Paquete Chicharrón Salsas negras': {
    categoria: 'Chicharrón',
    contenido: { 'Chicharrón Salsas negras': 12 }
  },
  'Paquete Chicharrón Flaming hot': {
    categoria: 'Chicharrón',
    contenido: { 'Chicharrón Flaming hot': 12 }
  },
  'Paquete Chicharrón Mix': {
    categoria: 'Chicharrón',
    contenido: {
      'Chicharrón Sal y limón': 2,
      'Chicharrón Salsas negras': 2,
      'Chicharrón Flaming hot': 2
    }
  },
};

function piezasTotalesDePaquete(nombrePaquete, numPaquetes) {
  const paquete = PAQUETES[nombrePaquete];
  if (!paquete) return null;
  const totalPiezasPaquete = Object.values(paquete.contenido).reduce((s, c) => s + c, 0);
  return totalPiezasPaquete * numPaquetes;
}

function desglosePiezasDePaquete(nombrePaquete, numPaquetes) {
  const paquete = PAQUETES[nombrePaquete];
  if (!paquete) return {};
  const desglose = {};
  Object.entries(paquete.contenido).forEach(([sabor, cantidadPorPaquete]) => {
    desglose[sabor] = cantidadPorPaquete * numPaquetes;
  });
  return desglose;
}
function tieneMayoreo(nombreProducto) {
  return Object.prototype.hasOwnProperty.call(PRECIOS_MAYOREO, nombreProducto);
}

// ============================================================
// DETECCIÓN AUTOMÁTICA DE NIVEL SEGÚN CANTIDAD DE PIEZAS
// ============================================================
function detectarNivelPorCantidad(piezas) {
  if (piezas >= 1201) return 'mayoreo3';
  if (piezas >= 601) return 'mayoreo2';
  if (piezas >= 121) return 'mayoreo1';
  return 'normal';
}
const RANGOS_MAYOREO = {
  normal:   { desde: 1,    hasta: 120 },
  mayoreo1: { desde: 121,  hasta: 600 },
  mayoreo2: { desde: 601,  hasta: 1200 },
  mayoreo3: { desde: 1201, hasta: null },
};

function textoRango(nivel) {
  const r = RANGOS_MAYOREO[nivel];
  if (!r) return '';
  return r.hasta ? `${r.desde}–${r.hasta} pzas` : `${r.desde}+ pzas`;
}

// ============================================================
// CALCULADORA DE MAYOREO / BONIFICACIÓN (Fase 2)
// ============================================================
function calcularPedidoMayoreo({ nombreProducto, cantidad, unidad, nivel, modo }) {
  const precios = PRECIOS_MAYOREO[nombreProducto];
  if (!precios) {
    return { esValido: false, mensajeError: 'Este producto no tiene precios de mayoreo configurados.' };
  }

  const piezasBase = unidad === 'paquetes' ? cantidad * piezasPorPaqueteDe(nombreProducto) : cantidad;

  // Determinar qué nivel se usa según el modo
  let nivelUsado = nivel || 'normal';
  if (modo === 'automatico') {
    nivelUsado = detectarNivelPorCantidad(piezasBase);
  }

  const precioPieza = precios[nivelUsado] ?? precios.normal;
  const totalCobrar = piezasBase * precioPieza;

  // Etiqueta informativa de oferta — SOLO en modo manual y SOLO si el nivel no es "normal"
  // Nunca modifica el precio ni el inventario, solo es una sugerencia para el vendedor.
  let sugerencia = null;
  if (modo === 'manual' && nivelUsado !== 'normal' && piezasBase > 0) {
    const precioNormal = precios.normal;
    // Si en vez de mayoreo cobraras precio normal por la cantidad pedida, ¿cuántas piezas
    // podrías entregar en total (dando algunas de regalo) sin bajar del ingreso del mayoreo?
    // Redondeamos siempre hacia ABAJO para no pasarnos del ingreso del mayoreo (beneficio del vendedor)
    const ingresoSiCobroNormal = piezasBase * precioNormal;
    const piezasEquivalentes = Math.floor(ingresoSiCobroNormal / precioPieza);
    const piezasRegalo = Math.max(0, piezasEquivalentes - piezasBase);
    if (piezasRegalo > 0) {
      const piezasPorPaquete = piezasPorPaqueteDe(nombreProducto);
      const paquetesRegalo = Math.floor(piezasRegalo / piezasPorPaquete);
      const piezasRegaloSueltas = piezasRegalo % piezasPorPaquete;
      sugerencia = {
        piezasRegalo,
        paquetesRegalo,
        piezasRegaloSueltas,
  ...
  return {
    esValido: true,
    nivelUsado,
    piezasVendidas: piezasBase,
    piezasTotalesInventario: piezasBase,
    totalCobrar,
    sugerencia,
  };
}
...
async function cargarCatalogo() {
  ...
  } else if (res.success && res.productos.length === 0) {
    // Firestore respondió bien y de verdad está vacío (primera vez real) -> ahí sí guardamos defaults
    await Catalogo.guardar(PRODUCTOS_CATALOGO);
  } else {
    // Hubo un error de conexión/permiso: NO tocar Firestore, solo usar la lista local
    // temporalmente para que la app no se caiga, pero sin borrar tu catálogo real.
    console.warn('No se pudo leer el catálogo (posible falla de red). No se modificó Firestore.');
    showToast('⚠️ No se pudo cargar el catálogo. Revisa tu conexión y reinicia la app.', 'error');
  }
}
...
// PRODUCTOS EN PEDIDO
// ============================================================
function renderControlesMayoreo(item, idx) {
  const precios = PRECIOS_MAYOREO[item.producto];
  const esMix = esPaqueteMix(item.producto);
  const unidad = esMix ? 'paquetes' : (item.unidadCaptura || 'piezas');
  const nivel = item.nivelMayoreo || 'normal';
  const piezasPorPaquete = piezasPorPaqueteDe(item.producto);
  const modo = item.modoMayoreo || 'automatico';
  const cantidadCapturada = item.cantidadCapturada || '';

  const resultado = cantidadCapturada > 0
    ? calcularPedidoMayoreo({ nombreProducto: item.producto, cantidad: parseFloat(cantidadCapturada) || 0, unidad, nivel, modo })
    : null;

  const nombresNivel = { normal: 'Normal', mayoreo1: 'Mayoreo 1', mayoreo2: 'Mayoreo 2', mayoreo3: 'Mayoreo 3' };
  const tablaReferenciaHTML = `
    <div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;
      padding:0.5rem 0.7rem;margin-bottom:0.5rem;font-size:0.74rem;">
      <div style="font-weight:700;color:var(--text-muted);margin-bottom:0.3rem;">📊 Rangos de mayoreo</div>
      ${Object.keys(nombresNivel).map(n => `
        <div style="display:flex;justify-content:space-between;${nivel === n ? 'color:var(--accent2);font-weight:700;' : 'color:var(--text-muted);'}">
          <span>${nombresNivel[n]}</span>
          <span>${textoRango(n)} · $${precios[n]}/pza</span>
        </div>
      `).join('')}
    </div>`;
  ...
  if (resultado && resultado.esValido) {
    const nivelDetectadoTexto = modo === 'automatico'
      ? `<div style="color:var(--text-muted);">📊 Nivel detectado: <strong>${nombresNivel[resultado.nivelUsado]}</strong></div>`
      : '';

    resumenHTML = `
      <div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:8px;padding:0.5rem 0.7rem;margin-bottom:0.5rem;font-size:0.78rem;">
        <div>💰 Total a cobrar: <strong>${resultado.totalCobrar.toLocaleString('es-MX',{style:'currency',currency:'MXN'})}</strong></div>
        ${nivelDetectadoTexto}
        <div style="color:var(--text-muted);">📦 Sale de bodega: ${resultado.piezasTotalesInventario} piezas</div>
      </div>`;

    if (resultado.sugerencia) {
      const s = resultado.sugerencia;
      const textoRegalo = s.paquetesRegalo > 0
        ? `${s.paquetesRegalo} paquete(s) + ${s.piezasRegaloSueltas} pieza(s) (${s.piezasRegalo} piezas en total)`
        : `${s.piezasRegalo} pieza(s)`;
      resumenHTML += `
        <div style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.3);border-radius:8px;padding:0.5rem 0.7rem;margin-bottom:0.5rem;font-size:0.76rem;color:#93c5fd;">
          💡 <strong>Sugerencia de oferta:</strong> si cobras a precio normal y entregas ${textoRegalo} de regalo (${s.piezasEquivalentes} piezas en total), obtienes prácticamente el mismo ingreso que con este nivel de mayoreo. Esto es solo informativo, no cambia el cobro ni el inventario.
        </div>`;
    }
  }
  ...
  return `
    <div class="form-row">
      <div class="form-group" style="margin-bottom:0.4rem;">
        <label style="font-size:0.72rem;">Unidad</label>
        ${esMix
          ? `<div style="padding:0.5rem;font-size:0.85rem;color:var(--text-muted);">Paquetes (${piezasPorPaquete} pzas) — Mix</div>`
          : `<select onchange="actualizarProductoMayoreo(${idx},'unidadCaptura',this.value)">
              <option value="piezas" ${unidad === 'piezas' ? 'selected' : ''}>Piezas</option>
              <option value="paquetes" ${unidad === 'paquetes' ? 'selected' : ''}>Paquetes (${piezasPorPaquete} pzas)</option>
            </select>`
        }
      </div>
      <div class="form-group" style="margin-bottom:0.4rem;">
        <label style="font-size:0.72rem;">Cantidad (${unidad})</label>
        <input type="number" min="1" value="${cantidadCapturada}" placeholder="0" inputmode="numeric"
          onchange="actualizarProductoMayoreo(${idx},'cantidadCapturada',this.value)" />
      </div>
    </div>
    <div class="form-row">
      <div class="form-group" style="margin-bottom:0.4rem;">
        <label style="font-size:0.72rem;">Cálculo de precio</label>
        <select onchange="actualizarProductoMayoreo(${idx},'modoMayoreo',this.value)">
          <option value="automatico" ${modo === 'automatico' ? 'selected' : ''}>Automático (según cantidad)</option>
          <option value="manual" ${modo === 'manual' ? 'selected' : ''}>Manual (yo elijo el nivel)</option>
        </select>
      </div>
      ${modo === 'manual' ? `
      <div class="form-group" style="margin-bottom:0.4rem;">
        <label style="font-size:0.72rem;">Nivel de precio</label>
        <select onchange="actualizarProductoMayoreo(${idx},'nivelMayoreo',this.value)">
          ${Object.keys(nombresNivel).map(n => `<option value="${n}" ${nivel === n ? 'selected' : ''}>${nombresNivel[n]} ($${precios[n]}/pza)</option>`).join('')}
        </select>
      </div>` : ''}
    </div>
    ${tablaReferenciaHTML}
...
function renderProductosEnPedido() {
  ...
  container.innerHTML = App.productosEnPedido.map((item, idx) => {
    const esMayoreo = tieneMayoreo(item.producto);

    const controlesProducto = esMayoreo
      ? renderControlesMayoreo(item, idx)
      : `
      <div class="form-row">
        <div class="form-group" style="margin-bottom:0.4rem;">
...
window.actualizarProductoMayoreo = function (idx, campo, valor) {
  const item = App.productosEnPedido[idx];
  if (!item) return;

  item[campo] = campo === 'cantidadCapturada' ? (parseFloat(valor) || 0) : valor;

  const unidad = esPaqueteMix(item.producto) ? 'paquetes' : (item.unidadCaptura || 'piezas');
  const nivel = item.nivelMayoreo || 'normal';
  const modo = item.modoMayoreo || 'automatico';
  const cantidadCapturada = item.cantidadCapturada || 0;

  if (cantidadCapturada > 0) {
    const resultado = calcularPedidoMayoreo({ nombreProducto: item.producto, cantidad: cantidadCapturada, unidad, nivel, modo });
    if (resultado.esValido) {
      item.cantidad = resultado.piezasVendidas;
      item.precioUnitario = resultado.piezasVendidas > 0 ? resultado.totalCobrar / resultado.piezasVendidas : 0;
      item.piezasTotalesInventario = resultado.piezasTotalesInventario;
    }
  } else {
    item.cantidad = 0;
    item.precioUnitario = 0;
    item.piezasTotalesInventario = 0;
  }
...
window.actualizarProductoYPrecio = function (idx, nombreProducto) {
  ...
  if (tieneMayoreo(nombreProducto)) {
    App.productosEnPedido[idx].unidadCaptura = 'piezas';
    App.productosEnPedido[idx].nivelMayoreo = 'normal';
    App.productosEnPedido[idx].modoMayoreo = 'automatico';
    App.productosEnPedido[idx].cantidadCapturada = 0;
    App.productosEnPedido[idx].cantidad = 0;
    App.productosEnPedido[idx].precioUnitario = 0;
    App.productosEnPedido[idx].piezasTotalesInventario = 0;
  } else {
...
const tipoVenta = document.querySelector('input[name="tipoVenta"]:checked')?.value || 'directa';
  ...
  try {
    ...
    const fechaPedido = document.getElementById('fecha-pedido')?.value || fechaLocalHoy();
    ...
    if (tipoVenta === 'muestra' && result.id) {
      ...
      const movimientosMuestra = expandirProductosParaInventario(productosValidos, pr => pr.cantidad);
      movimientosMuestra.forEach(m => {
        stockActual[m.producto] = (stockActual[m.producto] || 0) - m.cantidad;
      });
      await Inventario.guardar(stockActual);
      await Inventario.registrarMovimiento('muestra', movimientosMuestra, `Muestra: ${muestraProspecto || 'sin nombre'}`);
    }
...
window.confirmarEntrega = async function () {
  ...
  try {
    ...
    // Traducir productos (expandiendo paquetes Mix en sus sabores reales)
    const movimientosEntrega = expandirProductosParaInventario(productosEntregados, pr => pr.cantidadEntregada || 0);

    // Verificar stock suficiente antes de entregar
    const stockActual = { ...App.inventarioStock };
    const sinStock = movimientosEntrega.filter(m => (stockActual[m.producto] || 0) < m.cantidad);
    if (sinStock.length > 0) {
      const nombres = sinStock.map(m => `${m.producto} (disponible: ${stockActual[m.producto] || 0})`).join(', ');
      if (!confirm(`⚠️ Stock insuficiente para: ${nombres}\n\n¿Continuar de todas formas?`)) return;
    }
    ...
    movimientosEntrega.forEach(m => {
      stockActual[m.producto] = (stockActual[m.producto] || 0) - m.cantidad;
    });
    await Inventario.guardar(stockActual);
    await Inventario.registrarMovimiento('salida', movimientosEntrega, `Entrega pedido ${pedidoEnModal}`);
...
window.abrirModalCobro = function (id) {
  ...
  if (secDev) {
    ...
    const productosModal = pedido.productosEntregados || pedido.productos || [];
    secDev.innerHTML = `
      <div style="font-size:0.8rem;font-weight:700;color:var(--text-muted);margin-bottom:0.5rem;">📦 Este cobro (solo lo nuevo de hoy)</div>
      ${productosModal.map((pr, i) => {
        const cantEntregada = pr.cantidadEntregada ?? pr.cantidad ?? 0;
        const devPrev = pedido.detalleDevuelto?.find(d => d.producto === pr.producto)?.cantidad || 0;
        const vendPrev = pedido.detalleVendido?.find(d => d.producto === pr.producto)?.cantidad || 0;
        const pendienteHastaAhora = cantEntregada - devPrev - vendPrev;
        return `
        <div style="margin-bottom:0.6rem;padding-bottom:0.5rem;border-bottom:1px solid var(--border);" data-prev-vend="${vendPrev}" data-prev-dev="${devPrev}" data-cant-entregada="${cantEntregada}">
          <div style="display:flex;justify-content:space-between;font-size:0.82rem;margin-bottom:0.3rem;">
            <span>${pr.producto}</span>
            <span style="color:var(--text-muted);font-size:0.75rem;">Pendiente hasta hoy: <strong>${pendienteHastaAhora}</strong> @ $${pr.precioUnitario}</span>
          </div>
          <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.76rem;">
            <div style="flex:1;">
              <label style="color:#4ade80;">✅ Vendiendo hoy</label>
              <input type="number" min="0" max="${pendienteHastaAhora}" placeholder="0"
                id="cobro-vend-cant-${i}"
                inputmode="numeric"
                value=""
                oninput="calcularSaldoCobro()"
                style="width:100%;padding:0.3rem 0.5rem;border-radius:6px;border:1px solid var(--border);
                background:var(--surface);color:var(--text);font-size:0.85rem;text-align:center;" />
            </div>
            <div style="flex:1;">
              <label style="color:#f87171;">↩️ Devolviendo hoy</label>
              <input type="number" min="0" max="${pendienteHastaAhora}" placeholder="0"
                id="cobro-dev-cant-${i}"
                inputmode="numeric"
                value=""
            ...
            <div style="flex:1;text-align:center;">
              <label style="color:var(--text-muted);">⏳ Quedará pendiente</label>
              <div id="cobro-sindefinir-${i}" style="font-weight:700;padding-top:0.3rem;">${pendienteHastaAhora}</div>
            </div>
          </div>
        </div>`;
      }).join('')}
      <div id="modal-cobro-devueltas-info" style="font-size:0.78rem;color:#f59e0b;margin-top:0.4rem;white-space:pre-line;"></div>
    `;
  }
...
window.calcularSaldoCobro = function () {
  ...
  let montoVendidoHoy = 0;
  let montoDevueltoHoy = 0;
  let detalleVendidoHoy = '';
  let detalleDevueltoHoy = '';

  productos.forEach((pr, i) => {
    const bloque = document.getElementById(`cobro-vend-cant-${i}`)?.closest('[data-prev-vend]');
    if (!bloque) return;
    const cantEntregada = parseFloat(bloque.dataset.cantEntregada) || 0;
    const prevVend = parseFloat(bloque.dataset.prevVend) || 0;
    const prevDev = parseFloat(bloque.dataset.prevDev) || 0;
    const precio = pr.precioUnitario || 0;

    const cantVendHoy = parseFloat(document.getElementById(`cobro-vend-cant-${i}`)?.value) || 0;
    const cantDevHoy = parseFloat(document.getElementById(`cobro-dev-cant-${i}`)?.value) || 0;

    montoVendidoHoy += cantVendHoy * precio;
    montoDevueltoHoy += cantDevHoy * precio;

    if (cantVendHoy > 0) detalleVendidoHoy += `✅ ${pr.producto}: ${cantVendHoy} × $${precio}  `;
    if (cantDevHoy > 0) detalleDevueltoHoy += `↩️ ${pr.producto}: ${cantDevHoy} × $${precio}  `;

    const pendienteEl = document.getElementById(`cobro-sindefinir-${i}`);
    if (pendienteEl) {
      const quedaPendiente = cantEntregada - (prevVend + cantVendHoy) - (prevDev + cantDevHoy);
      pendienteEl.textContent = quedaPendiente;
      pendienteEl.style.color = quedaPendiente < 0 ? '#f87171' : quedaPendiente === 0 ? '#4ade80' : 'inherit';
    }
  });

  const montoInput = document.getElementById('modal-cobro-monto');
  if (montoInput && !montoInput.dataset.editadoManual) {
    montoInput.value = montoVendidoHoy > 0 ? montoVendidoHoy.toFixed(2) : '';
  }

  const devInfo = document.getElementById('modal-cobro-devueltas-info');
  if (devInfo) {
    let texto = '';
    if (detalleVendidoHoy) texto += detalleVendidoHoy + '\n';
    if (detalleDevueltoHoy) texto += detalleDevueltoHoy + '\n';
    texto += `Cobrando hoy: ${montoVendidoHoy.toLocaleString('es-MX',{style:'currency',currency:'MXN'})}`;
    devInfo.textContent = texto;
  }
};

document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('modal-cobro-monto')?.addEventListener('input', function() {
...
window.confirmarCobro = async function () {
  ...
  let montoDevueltoHoy = 0, unidadesDevueltasHoy = 0;
  const detalleVendidoHoy = [];
  const detalleDevueltoHoy = [];

  productos.forEach((pr, i) => {
    const cantVendHoy = parseFloat(document.getElementById(`cobro-vend-cant-${i}`)?.value) || 0;
    const cantDevHoy = parseFloat(document.getElementById(`cobro-dev-cant-${i}`)?.value) || 0;
    if (cantVendHoy > 0) detalleVendidoHoy.push({ producto: pr.producto, cantidad: cantVendHoy });
    if (cantDevHoy > 0) {
      detalleDevueltoHoy.push({ producto: pr.producto, cantidad: cantDevHoy });
      montoDevueltoHoy += cantDevHoy * (pr.precioUnitario || 0);
      unidadesDevueltasHoy += cantDevHoy;
    }
  });

  if (montoPagando <= 0 && unidadesDevueltasHoy <= 0) {
    showToast('Marca algo vendido o devuelto', 'error');
    return;
  }
  ...
  const totalEntregado = pedido.totalEntregado || pedido.total || 0;
  const yaCobrado = pedido.montoCobrado || 0;
  const yaDevuelto = pedido.montoDevuelto || 0;

  const nuevoTotalCobrado = yaCobrado + montoPagando;
  const nuevoTotalDevuelto = yaDevuelto + montoDevueltoHoy;
  const nuevoSaldo = totalEntregado - nuevoTotalDevuelto - nuevoTotalCobrado;

  const mergeDetalle = (prev, hoy) => {
    const mapa = {};
    (prev || []).forEach(d => mapa[d.producto] = (mapa[d.producto] || 0) + d.cantidad);
    hoy.forEach(d => mapa[d.producto] = (mapa[d.producto] || 0) + d.cantidad);
    return Object.entries(mapa).map(([producto, cantidad]) => ({ producto, cantidad }));
  };
  const nuevoDetalleVendido = mergeDetalle(pedido.detalleVendido, detalleVendidoHoy);
  const nuevoDetalleDevuelto = mergeDetalle(pedido.detalleDevuelto, detalleDevueltoHoy);

  const todoResuelto = productos.every(pr => {
    const cantEntregada = pr.cantidadEntregada ?? pr.cantidad ?? 0;
    const vend = nuevoDetalleVendido.find(d => d.producto === pr.producto)?.cantidad || 0;
    const dev = nuevoDetalleDevuelto.find(d => d.producto === pr.producto)?.cantidad || 0;
    return (cantEntregada - vend - dev) <= 0;
  });

  const datos = {
    montoCobrado: nuevoTotalCobrado,
    montoDevuelto: nuevoTotalDevuelto > 0 ? nuevoTotalDevuelto : null,
    unidadesDevueltas: (pedido.unidadesDevueltas || 0) + unidadesDevueltasHoy || null,
    detalleVendido: nuevoDetalleVendido,
    detalleDevuelto: nuevoDetalleDevuelto,
    deuda: nuevoSaldo > 0.01 ? nuevoSaldo : 0,
    estado: (todoResuelto || nuevoSaldo <= 0.01) ? 'pagado' : 'parcial',
    notasCobro: notas,
    ...((todoResuelto || nuevoSaldo <= 0.01) ? { fechaCierre: fechaLocalHoy() } : {}),
  };
  ...
  try {
    ...
    if (montoPagando > 0 || unidadesDevueltasHoy > 0) {
      await Pedidos.registrarPago(pedidoEnModal, {
        monto: montoPagando,
        unidadesDevueltas: unidadesDevueltasHoy,
        montoDevuelto: montoDevueltoHoy,
        detalleVendido: detalleVendidoHoy,
        detalleDevuelto: detalleDevueltoHoy,
        origen: 'delta',
        notas,
      });
    }

    if (unidadesDevueltasHoy > 0) {
      const stockActual = { ...App.inventarioStock };
      const movimientos = expandirProductosParaInventario(detalleDevueltoHoy, pr => pr.cantidad);
      movimientos.forEach(m => {
        stockActual[m.producto] = (stockActual[m.producto] || 0) + m.cantidad;
      });
      await Inventario.guardar(stockActual);
      await Inventario.registrarMovimiento('devolucion', movimientos, `Devolución cobro ${pedidoEnModal}`);
    }

    showToast(datos.estado === 'pagado' ? '✅ Consignación liquidada' : '💰 Pago registrado', 'success');
    cerrarModalCobro();
  } catch (err) {
    showToast('Error: ' + err.message, 'error');
  }
...
window.cambiarEstado = async function (id, nuevoEstado) {
  ...
  if (estabaEntregado && (seEstaCancel || seEstaPendiente)) {
    ...
    if (tieneEntregados) {
      const stockActual = { ...App.inventarioStock };
      const movimientos = expandirProductosParaInventario(productosEntregados, pr => pr.cantidadEntregada ?? pr.cantidad ?? 0);
      movimientos.forEach(m => {
        stockActual[m.producto] = (stockActual[m.producto] || 0) + m.cantidad;
      });

      await Inventario.guardar(stockActual);
      await Inventario.registrarMovimiento('devolucion', movimientos, `Cancelación/reversión pedido ${id}`);
...
window.eliminarPedido = async function (id) {
  ...
  if (pedido && estadosConStock.includes(pedido.estado)) {
    ...
    if (tieneEntregados) {
      ...
      await Inventario.guardar(stockActual);
      await Inventario.registrarMovimiento('devolucion', movimientos, `Eliminación pedido ${id} (${pedido.tipoVenta || 'directa'})`);
    }
  }
...
App.unsubscribeClientes = Clientes.escuchar((clientes) => {
    ...
    renderClientes();
    const selectEnfocado = document.activeElement?.id === 'pedido-cliente-id';
    if (!selectEnfocado) poblarSelectClientes();
...
function renderPedidos() {
  ...
  container.innerHTML = lista.map(p => {
    ...
    const productosStr = Array.isArray(p.productos)
      ? p.productos.map(pr => 
          `<div style="display:block;padding:3px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
            <span style="font-weight:700;color:#f5a623;margin-right:6px;">${pr.cantidad}x</span><span>${pr.producto}</span>
          </div>`
        ).join('')
      : (p.producto ? `×${p.cantidad} ${p.producto}` : '—');
      ...
      <div class="pedido-card" data-id="${p.id}">
        <div class="pedido-header">
          <div>
            ...
            <div class="pedido-meta" style="padding:4px 0;">${productosStr}</div>
            ${deudaBadge}
          </div>
        ...
        <div class="pedido-actions">
          ...
          <button class="btn btn-outline btn-sm" onclick="editarFechaEntrega('${p.id}')">📅 Fecha</button>
...
${c.ubicacion ? `<div class="cliente-detalle" style="display:flex;align-items:center;gap:0.5rem;">
  <span style="color:var(--accent2);">📍 Ubicación guardada</span>
  <button onclick="compartirUbicacionCliente('${(c.nombre||c.escuela||'').replace(/'/g,'')}'
    ,${c.ubicacion.lat},${c.ubicacion.lng},event)"
    style="padding:2px 10px;border-radius:20px;border:none;background:rgba(37,211,102,0.15);
    color:#25d366;font-size:0.72rem;font-weight:700;cursor:pointer;">
    📤 Compartir
  </button>
</div>` : ''}
      </div>
...
function actualizarStatTotalVendido(todos) {
  ...
  const total = pedidos.filter(p => p.tipoVenta !== 'muestra').reduce((s, p) => {
...
function actualizarStatCobrado(todos) {
  ...
  const cobrado = pedidos.filter(p => p.estado === 'pagado' && p.tipoVenta !== 'muestra').reduce((s, p) => {
...
function actualizarStatTopProducto(todos) {
  ...
  const conteo = {};
  pedidos.filter(p => p.tipoVenta !== 'muestra').forEach(p => {
...
function obtenerPendientesOrdenados() {
  ...
  if (!App.ubicacionBodega && pendientes.length > 0) {
    showToast('⚠️ Sin ubicación de bodega configurada. Ruta sin optimizar.', 'error');
  }
  if (App.ubicacionBodega && pendientes.length > 0) {
...
window.compartirUbicacionCliente = function(nombre, lat, lng, event) {
  event.stopPropagation();
  const url = `https://maps.google.com/?q=${lat},${lng}`;
  const texto = `📍 Ubicación de ${nombre}:\n${url}`;

  const overlay = document.createElement('div');
  overlay.className = 'overlay-ubicacion-sheet';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:9998;background:rgba(0,0,0,0.4);';

  const sheet = document.createElement('div');
  sheet.id = 'sheet-ubicacion';
  sheet.style.cssText = `
  ...
  sheet.innerHTML = `
    <div style="width:40px;height:4px;background:var(--border);border-radius:2px;margin:0 auto 1rem;"></div>
    <div style="font-size:0.85rem;font-weight:700;margin-bottom:0.25rem;">${nombre}</div>
    <div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:1rem;">
      📍 ${lat.toFixed(5)}, ${lng.toFixed(5)}
    </div>
    <div style="display:flex;flex-direction:column;gap:0.6rem;">
      <button id="btn-sheet-whatsapp"
        style="padding:0.75rem;border-radius:12px;border:none;
        background:#25d366;color:white;font-size:0.9rem;font-weight:700;cursor:pointer;">
        💬 Compartir por WhatsApp
      </button>
      <button id="btn-sheet-copiar"
        style="padding:0.75rem;border-radius:12px;border:1px solid var(--border);
        background:var(--surface2);color:var(--text);font-size:0.9rem;font-weight:700;cursor:pointer;">
        🔗 Copiar enlace
      </button>
      <button id="btn-sheet-cancelar"
        style="padding:0.65rem;border-radius:12px;border:none;
        background:transparent;color:var(--text-muted);font-size:0.85rem;cursor:pointer;">
        Cancelar
      </button>
    </div>`;

  overlay.onclick = cerrarSheet;
  document.body.appendChild(overlay);
  document.body.appendChild(sheet);

  document.getElementById('btn-sheet-whatsapp').onclick = function() {
    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(texto)}`, '_blank');
    cerrarSheet();
  };
  document.getElementById('btn-sheet-copiar').onclick = function() {
    navigator.clipboard.writeText(url).then(() => showToast('Enlace copiado ✓', 'success'));
    cerrarSheet();
  };
  document.getElementById('btn-sheet-cancelar').onclick = cerrarSheet;
};
...
function resetFormPedido() {
  ...
  if (fechaEl) fechaEl.value = fechaLocalHoy();
}
...
const todosPedidos = await Pedidos.obtenerPorFecha(inicio, fin);

const inicioDate = new Date(inicio + 'T00:00:00');
const finDate = new Date(fin + 'T23:59:59');

// Ya viene filtrado por fecha de entrega desde Firestore
...
// ── DATOS RESUMEN ──
  // Ventas directas del periodo (siempre pagadas completas al entregar)
const ventasDirectasPeriodo = pedidos.filter(p => p.tipoVenta !== 'consignacion' && p.tipoVenta !== 'muestra');
const totalVentasDirectas = ventasDirectasPeriodo.reduce((s,p) => s + (p.montoCobrado || p.totalEntregado || p.total || 0), 0);

// Consignas entregadas en el periodo -> lo que aún deben = Pendiente
const consignasDelPeriodo = pedidos.filter(p => p.tipoVenta === 'consignacion');
const totalPendiente = consignasDelPeriodo.reduce((s,p) => s + (p.deuda || 0), 0);

  // ── TABLA MAESTRA INVENTARIO — se llena en dos pasadas ──
  const mapaInv = {};
  catalogoSinPaquetes().forEach(p => {
    const nombre = typeof p === 'string' ? p : p.nombre;
    mapaInv[nombre] = { directa:0, consigna:0, consignaCobrada:0, devuelto:0, muestras:0, importe:0 };
  });

  // Pagos de CUALQUIER consigna activa, cuya fecha de PAGO cae en el periodo
  // (sin importar cuándo se entregó esa consigna). Esta es la ÚNICA fuente de
  // verdad para "dinero vendido" y "devuelto" de consignaciones.
  const todasLasConsignas = App.pedidosList.filter(p =>
    p.tipoVenta === 'consignacion' && ['entregado','parcial','pagado'].includes(p.estado)
  );
  let totalConsignasCobradasPeriodo = 0;
  const pagosConsignaEnPeriodo = [];

  for (const p of todasLasConsignas) {
    const resPagos = await Pedidos.obtenerPagos(p.id);
    const pagos = resPagos.pagos || [];
    const productosPedido = p.productosEntregados || p.productos || [];
    const totalEntregadoPedido = p.totalEntregado || p.total || 0;

    pagos.forEach(pago => {
      const fechaPago = pago.fecha?.toDate ? pago.fecha.toDate() : new Date(pago.fecha);
      if (!(fechaPago >= inicioDate && fechaPago <= finDate)) return;

      totalConsignasCobradasPeriodo += pago.monto || 0;
      pagosConsignaEnPeriodo.push({
        clienteNombre: p.clienteNombre || '—',
        fechaTexto: pago.fechaTexto || fechaPago.toLocaleDateString('es-MX'),
        monto: pago.monto || 0,
        montoDevuelto: pago.montoDevuelto || 0,
        notas: pago.notas || '',
      });

      // $ vendido de este pago, repartido por producto
      if (Array.isArray(pago.detalleVendido) && pago.detalleVendido.length > 0) {
        pago.detalleVendido.forEach(dv => {
          const prodInfo = productosPedido.find(pr => pr.producto === dv.producto);
          const precio = prodInfo?.precioUnitario || 0;
          const desglose = esPaqueteMix(dv.producto)
            ? expandirProductoParaInventario(dv.producto, dv.cantidad)
            : [{ producto: dv.producto, cantidad: dv.cantidad }];
          desglose.forEach(({ producto: nombreReal, cantidad: cant }) => {
            if (!mapaInv[nombreReal]) mapaInv[nombreReal] = { directa:0, consigna:0, consignaCobrada:0, devuelto:0, muestras:0, importe:0 };
            mapaInv[nombreReal].importe += cant * precio;
            mapaInv[nombreReal].consignaCobrada += cant;
          });
        });
      } else if ((pago.monto || 0) > 0 && totalEntregadoPedido > 0) {
        // Pagos antiguos sin detalle: repartir proporcionalmente por valor de cada producto
        productosPedido.forEach(pr => {
          const cantTotal = pr.cantidadEntregada ?? pr.cantidad ?? 0;
          const precio = pr.precioUnitario ?? 0;
          const valorProducto = cantTotal * precio;
          const proporcion = valorProducto / totalEntregadoPedido;
          const importeAsignado = pago.monto * proporcion;
          const desglose = esPaqueteMix(pr.producto)
            ? expandirProductoParaInventario(pr.producto, cantTotal)
            : [{ producto: pr.producto, cantidad: cantTotal }];
          const totalCantDesglose = desglose.reduce((s,d)=>s+d.cantidad,0) || 1;
          desglose.forEach(({ producto: nombreReal, cantidad: cant }) => {
            if (!mapaInv[nombreReal]) mapaInv[nombreReal] = { directa:0, consigna:0, consignaCobrada:0, devuelto:0, muestras:0, importe:0 };
            mapaInv[nombreReal].importe += importeAsignado * (cant / totalCantDesglose);
            mapaInv[nombreReal].consignaCobrada += cant * (cant / totalCantDesglose);
          });
        });
      }

      // Piezas devueltas de este pago, repartidas por producto
      if (Array.isArray(pago.detalleDevuelto) && pago.detalleDevuelto.length > 0) {
        pago.detalleDevuelto.forEach(dd => {
          const desglose = esPaqueteMix(dd.producto)
            ? expandirProductoParaInventario(dd.producto, dd.cantidad)
            : [{ producto: dd.producto, cantidad: dd.cantidad }];
          desglose.forEach(({ producto: nombreReal, cantidad: cant }) => {
            if (!mapaInv[nombreReal]) mapaInv[nombreReal] = { directa:0, consigna:0, consignaCobrada:0, devuelto:0, muestras:0, importe:0 };
            mapaInv[nombreReal].devuelto += cant;
          });
        });
      } else if ((pago.unidadesDevueltas || 0) > 0) {
        // Pagos antiguos sin detalle: repartir proporcionalmente por piezas entregadas
        const totalPiezasPedido = productosPedido.reduce((s,pr) => s + (pr.cantidadEntregada ?? pr.cantidad ?? 0), 0) || 1;
        productosPedido.forEach(pr => {
          const cantTotal = pr.cantidadEntregada ?? pr.cantidad ?? 0;
          const proporcion = cantTotal / totalPiezasPedido;
          const piezasAsignadas = pago.unidadesDevueltas * proporcion;
          const desglose = esPaqueteMix(pr.producto)
            ? expandirProductoParaInventario(pr.producto, cantTotal)
            : [{ producto: pr.producto, cantidad: cantTotal }];
          const totalCantDesglose = desglose.reduce((s,d)=>s+d.cantidad,0) || 1;
          desglose.forEach(({ producto: nombreReal, cantidad: cant }) => {
            if (!mapaInv[nombreReal]) mapaInv[nombreReal] = { directa:0, consigna:0, consignaCobrada:0, devuelto:0, muestras:0, importe:0 };
            mapaInv[nombreReal].devuelto += piezasAsignadas * (cant / totalCantDesglose);
          });
        });
      }
    });
  ...
  const totalMontoDevueltoConsignaPeriodo = pagosConsignaEnPeriodo.reduce((s,pg) => s + pg.montoDevuelto, 0);
  const totalCobrado = totalVentasDirectas + totalConsignasCobradasPeriodo;
  ...
  // Piezas que salieron de bodega (según fecha de ENTREGA), y $ vendido de DIRECTAS
  // (proporcional a lo cobrado si quedó deuda)
  pedidos.forEach(p => {
    ...
    prods.forEach(pr => {
      ...
      desglose.forEach(({ producto: nombreReal, cantidad: cant }) => {
        ...
        if (p.tipoVenta === 'muestra') {
          mapaInv[nombreReal].muestras += cant;
        } else if (p.tipoVenta === 'consignacion') {
          // El $ y las devoluciones de consignación ya se contaron arriba, por fecha de COBRO real.
          mapaInv[nombreReal].consigna += cant;
        } else {
          mapaInv[nombreReal].directa += cant;
          if (['pagado','parcial'].includes(p.estado)) {
            const totalEntregadoPedido = p.totalEntregado || p.total || 0;
            const proporcionCobrada = totalEntregadoPedido > 0
              ? Math.min(1, (p.montoCobrado || 0) / totalEntregadoPedido)
              : 0;
            mapaInv[nombreReal].importe += importe * proporcionCobrada;
          }
        }
      });
    });
  ...
  const filasInv = Object.entries(mapaInv).filter(([,v]) =>
    v.directa > 0 || v.consigna > 0 || v.muestras > 0 || v.consignaCobrada > 0
  );
  const totInv = filasInv.reduce((s,[,v]) => ({
    directa:  s.directa  + v.directa,
    consigna: s.consigna + v.consigna,
    consignaCobrada: s.consignaCobrada + v.consignaCobrada,
    devuelto: s.devuelto + v.devuelto,
    muestras: s.muestras + v.muestras,
    importe:  s.importe  + v.importe,
  }), { directa:0, consigna:0, consignaCobrada:0, devuelto:0, muestras:0, importe:0 });
   // ── CONSIGNACIONES ACTIVAS ──
  // Siempre TODAS las consignas activas, sin importar el periodo del reporte
const consigActivas = App.pedidosList.filter(p =>
  ...
  // ── RANKING CLIENTES ──
  const mapaClientes = {};
  pedidos.filter(p => ['pagado','parcial','entregado'].includes(p.estado) && p.tipoVenta !== 'muestra' && p.clienteId).forEach(p => {
...
@page {
  ...
  <div class="seccion">
    ...
    <table>
      <thead><tr>
        ...
        <th class="r">Consigna cobrada</th>
  ...
  if (filasInv.length === 0) {
    ...
    filasInv.forEach(([nombre, v]) => {
      ...
      html += `<tr>
        ...
        <td class="r" style="color:#22c55e;">${v.consignaCobrada > 0 ? v.consignaCobrada : '—'}</td>
  ...
  html += `</tbody>
    <tfoot><tr class="fila-total">
      ...
      <td class="r">${totInv.consignaCobrada}</td>
    ...
    <div class="pie-notas">
      <span>📌 <strong>V. Consigna</strong> — piezas entregadas a consigna, aún sin vender/cobrar.</span>
      <span>💵 <strong>Consigna cobrada</strong> — de esas piezas, cuántas ya se vendieron (pago total o parcial).</span>
  ...
  // ── 4B. CONSIGNACIONES COBRADAS EN EL PERÍODO ──
  html += `
  <div class="seccion">
    <div class="sec-header">
      <div class="sec-icono">💵</div>
      <div><div class="sec-titulo">Consignaciones cobradas en el período</div>
      <div class="sec-sub">Pagos recibidos entre ${inicioFmt} y ${finFmt}, sin importar cuándo se entregó el producto</div></div>
    </div>`;

  if (pagosConsignaEnPeriodo.length === 0) {
    html += `<p style="color:#94a3b8;font-size:11px;padding:8px 0;">Sin cobros de consignación registrados en este período.</p>`;
  } else {
    html += `<table><thead><tr>
      <th>Cliente</th><th>Fecha de cobro</th><th class="r">Cobrado</th><th class="r">Devuelto</th><th>Notas</th>
    </tr></thead><tbody>`;
    pagosConsignaEnPeriodo.forEach(pg => {
      html += `<tr>
        <td><strong>${pg.clienteNombre}</strong></td>
        <td style="font-size:10px;">${pg.fechaTexto}</td>
        <td class="r" style="color:#22c55e;font-weight:700;">${fmt(pg.monto)}</td>
        <td class="r" style="color:${pg.montoDevuelto > 0 ? '#f59e0b' : 'inherit'};">${pg.montoDevuelto > 0 ? fmt(pg.montoDevuelto) : '—'}</td>
        <td style="font-size:10px;color:#64748b;">${pg.notas || '—'}</td>
      </tr>`;
    });
    html += `</tbody><tfoot><tr class="fila-total">
      <td colspan="2">TOTAL COBRADO EN CONSIGNACIONES</td>
      <td class="r">${fmt(totalConsignasCobradasPeriodo)}</td>
      <td class="r">${fmt(totalMontoDevueltoConsignaPeriodo)}</td>
      <td></td>
    </tr></tfoot></table>`;
  }
  html += `</div>`;
...
// ── 8. INVENTARIO ACTUAL ──
const stockActual = App.inventarioStock;
const productosInv = catalogoSinPaquetes();
...
ventanaPDF.focus();
setTimeout(() => ventanaPDF.print(), 800);
}
...
function renderInventario() {
  ...
  const productos = catalogoSinPaquetes();
...
function showToast(msg, tipo = 'info') {
  ...
  if (!container) return;
  // Limitar a máximo 3 toasts visibles a la vez
...
window.editarCobro = function (id) {
  ...
  if (secDev) {
    ...
    const productosModal = pedido.productosEntregados || pedido.productos || [];
    secDev.innerHTML = `
      <div style="font-size:0.8rem;font-weight:700;color:var(--text-muted);margin-bottom:0.5rem;">📦 Estado por producto</div>
      ${productosModal.map((pr, i) => {
        const cantEntregada = pr.cantidadEntregada ?? pr.cantidad ?? 0;
        const devPrev = pedido.detalleDevuelto?.find(d => d.producto === pr.producto)?.cantidad || 0;
        const vendPrev = pedido.detalleVendido?.find(d => d.producto === pr.producto)?.cantidad || 0;
        const pendiente = cantEntregada - devPrev - vendPrev;
        return `
        <div style="margin-bottom:0.6rem;padding-bottom:0.5rem;border-bottom:1px solid var(--border);">
          <div style="display:flex;justify-content:space-between;font-size:0.82rem;margin-bottom:0.3rem;">
            <span>${pr.producto}</span>
            <span style="color:var(--text-muted);font-size:0.75rem;">×${cantEntregada} @ $${pr.precioUnitario}</span>
          </div>
          <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.76rem;">
            <div style="flex:1;">
              <label style="color:#4ade80;">✅ Vendido</label>
              <input type="number" min="0" max="${cantEntregada}" placeholder="0"
                id="cobro-vend-cant-${i}"
                inputmode="numeric"
                value="${vendPrev > 0 ? vendPrev : ''}"
                oninput="calcularSaldoCobro()"
                style="width:100%;padding:0.3rem 0.5rem;border-radius:6px;border:1px solid var(--border);
                background:var(--surface);color:var(--text);font-size:0.85rem;text-align:center;" />
            </div>
            <div style="flex:1;">
              <label style="color:#f87171;">↩️ Devuelto</label>
              <input type="number" min="0" max="${cantEntregada}" placeholder="0"
                id="cobro-dev-cant-${i}"
                inputmode="numeric"
                value="${devPrev > 0 ? devPrev : ''}"
                oninput="calcularSaldoCobro()"
                style="width:100%;padding:0.3rem 0.5rem;border-radius:6px;border:1px solid var(--border);
                background:var(--surface);color:var(--text);font-size:0.85rem;text-align:center;" />
            </div>
            <div style="flex:1;text-align:center;">
              <label style="color:var(--text-muted);">⏳ Pendiente</label>
              <div id="cobro-sindefinir-${i}" style="font-weight:700;padding-top:0.3rem;">${pendiente}</div>
            </div>
          </div>
        </div>`;
      }).join('')}
...
function exportarJSON() {
  const datos = {
    ...
    inventario: App.inventarioStock,
...
window.verHistorialCobros = async function (id) {
  ...
  const btnRecalcular = `<button onclick="recalcularPedidoManual('${id}')"
    style="width:100%;padding:0.5rem;margin-bottom:0.75rem;border-radius:10px;border:1px solid var(--accent2);
    background:transparent;color:var(--accent2);font-size:0.78rem;font-weight:700;cursor:pointer;">
    🔄 Recalcular este pedido con los pagos actuales
  </button>`;
  html += btnRecalcular;
      ...
      html += `
        <div style="display:flex;gap:0.75rem;margin-bottom:0.85rem;">
          ...
          <div style="flex:1;background:var(--surface2);border-radius:10px;padding:0.65rem 0.85rem;font-size:0.8rem;">
            ...
            <div style="font-size:0.72rem;color:var(--text-muted);margin-top:0.25rem;border-top:1px solid var(--border);padding-top:0.25rem;display:flex;justify-content:space-between;align-items:center;">
              <span>Acumulado: ${acumulado.toLocaleString('es-MX',{style:'currency',currency:'MXN'})}</span>
              <button onclick="eliminarPagoDelHistorial('${id}','${pago.id}')"
                style="border:none;background:rgba(248,113,113,0.15);color:#f87171;font-size:0.68rem;font-weight:700;
                padding:3px 8px;border-radius:8px;cursor:pointer;">🗑️ Eliminar pago</button>
            </div>
          </div>
        </div>`;
    });
...
window.eliminarPagoDelHistorial = async function (pedidoId, pagoId) {
  if (!confirm('¿Eliminar este pago del historial?\n\nEsto va a recalcular automáticamente el total cobrado, lo devuelto y el saldo de esta consignación con los pagos que queden.')) return;

  const resultado = await Pedidos.eliminarPago(pedidoId, pagoId);
  if (!resultado.success) {
    showToast('Error al eliminar: ' + resultado.error, 'error');
    return;
  }

  const recalculo = await Pedidos.recalcularDesdePagos(pedidoId);
  if (!recalculo.success) {
    showToast('Se eliminó el pago pero falló el recálculo: ' + recalculo.error, 'error');
    return;
  }

  showToast('Pago eliminado y saldo recalculado ✓', 'success');
  window.verHistorialCobros(pedidoId);
};
window.editarFechaEntrega = function (id) {
  const pedido = App.pedidosList.find(p => p.id === id);
  if (!pedido) return;

  const fechaActual = pedido.fechaEntregadoReal || fechaLocalHoy();
  ...
  sheet.innerHTML = `
    <div style="width:40px;height:4px;background:var(--border);border-radius:2px;margin:0 auto 1rem;"></div>
    <div style="font-size:0.95rem;font-weight:700;margin-bottom:0.25rem;">📅 Corregir fecha de entrega</div>
    <div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:1rem;">
      Cliente: ${pedido.clienteNombre || pedido.clienteEscuela || '—'}
    </div>
    <input type="date" id="input-fecha-entrega-corregida" value="${fechaActual}"
      style="width:100%;padding:0.7rem;border-radius:10px;border:1px solid var(--border);
      background:var(--surface2);color:var(--text);font-size:0.95rem;margin-bottom:1rem;" />
    <div style="display:flex;flex-direction:column;gap:0.6rem;">
      <button id="btn-guardar-fecha-entrega"
        style="padding:0.75rem;border-radius:12px;border:none;
        background:var(--accent2);color:#000;font-size:0.9rem;font-weight:700;cursor:pointer;">
        💾 Guardar fecha
      </button>
      <button id="btn-cancelar-fecha-entrega"
        style="padding:0.65rem;border-radius:12px;border:none;
        background:transparent;color:var(--text-muted);font-size:0.85rem;cursor:pointer;">
        Cancelar
      </button>
  ...
  document.getElementById('btn-cancelar-fecha-entrega').onclick = cerrar;

  document.getElementById('btn-guardar-fecha-entrega').onclick = async function () {
    const nueva = document.getElementById('input-fecha-entrega-corregida')?.value;
    if (!nueva) {
      showToast('Elige una fecha', 'error');
      return;
    }
    const result = await Pedidos.actualizar(id, { fechaEntregadoReal: nueva });
    if (result.success) {
      showToast('Fecha de entrega corregida ✓', 'success');
      cerrar();
    } else {
      showToast('Error: ' + result.error, 'error');
    }
...
window.recalcularPedidoManual = async function (pedidoId) {
  if (!confirm('¿Recalcular este pedido con los pagos que tiene actualmente?')) return;
  const resultado = await Pedidos.recalcularDesdePagos(pedidoId);
  if (!resultado.success) {
    showToast('Error: ' + resultado.error, 'error');
    return;
  }
  showToast('Pedido recalculado ✓', 'success');
  window.verHistorialCobros(pedidoId);
