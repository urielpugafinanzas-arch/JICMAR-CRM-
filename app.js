...
window.abrirModalCobro = function (id) {
  ...
  if (secDev) {
    ...
    secDev.innerHTML = `
      <div style="font-size:0.8rem;font-weight:700;color:var(--text-muted);margin-bottom:0.5rem;">↩️ Devoluciones por producto</div>
      ${productosModal.map((pr, i) => {
        ...
        <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.4rem;font-size:0.82rem;">
          <span style="flex:1;">${pr.producto}</span>
          ...
          <input type="number" min="0" max="${cantEntregada}" placeholder="0"
            ...
            style="width:65px;padding:0.3rem 0.5rem;border-radius:6px;border:1px solid var(--border);
            background:var(--surface);color:var(--text);font-size:0.85rem;text-align:center;" />
          ${devPrev > 0 ? `<span style="font-size:0.7rem;color:#f59e0b;">prev: ${devPrev}</span>` : ''}
        </div>`;
      }).join('')}
