...
window.calcularSaldoCobro = function () {
  ...
  if (devInfo) {
    if (montoDevuelto > 0 || montoVendido > 0) {
      ...
      texto += `Total a cobrar: ${totalRealACobrar.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}`;
...
const detalleVendido = productos.map((pr, i) => {
  ...
  try {
    ...
    if (!modoEdicionActual && (montoPagando > 0 || unidadesDevueltas > 0)) {
...
// (sin importar cuándo se entregó esa consigna)
...
for (const p of todasLasConsignas) {
  ...
  pagos.forEach(pago => {
    ...
    if (fechaPago >= inicioDate && fechaPago <= finDate) {
...
const totalPedidos = pedidos.length;
  ...
  pedidos.forEach(p => {
    ...
    prods.forEach(pr => {
      ...
      // Si es un Paquete Mix, repartir las piezas entre los sabores reales para el reporte
      ...
      desglose.forEach(({ producto: nombreReal, cantidad: cant }) => {
        ...
        if (p.tipoVenta === 'muestra') {
          ...
          if (['pagado','parcial'].includes(p.estado)) {
            const propDevuelto = (cant / cantTotal) * (p.montoDevuelto || 0);
            mapaInv[nombreReal].importe += importe - propDevuelto;
          }
        } else {
          mapaInv[nombreReal].directa += cant;
          if (['pagado','parcial'].includes(p.estado)) {
            mapaInv[nombreReal].importe += importe;
          }
        }
        if (p.unidadesDevueltas > 0 && p.tipoVenta === 'consignacion') {
          mapaInv[nombreReal].devuelto += (cant / cantTotal) * (p.unidadesDevueltas || 0);
        }
      });
    });
  });
