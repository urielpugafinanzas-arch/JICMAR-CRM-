...
import { Auth, Config, Catalogo, Pedidos, Clientes, Storage } from './firebase.js';
...
function renderCatalogo() {
  ...
  container.innerHTML = PRODUCTOS_CATALOGO.map((nombre, idx) => `
    <div style="display:flex;align-items:center;gap:0.6rem;padding:0.65rem 0.75rem;
      ...
      <span id="cat-texto-${idx}" style="flex:1;font-size:0.88rem;">${nombre}</span>
      <input id="cat-input-${idx}" type="text" value="${nombre}"
        style="display:none;flex:1;padding:0.3rem 0.5rem;font-size:0.85rem;border-radius:6px;
...
window.editarProductoCatalogo = function (idx) {
  document.getElementById(`cat-texto-${idx}`).style.display = 'none';
  document.getElementById(`cat-input-${idx}`).style.display = 'block';
  document.getElementById(`cat-btn-editar-${idx}`).style.display = 'none';
  document.getElementById(`cat-btn-guardar-${idx}`).style.display = 'inline-flex';
...
window.guardarProductoCatalogo = async function (idx) {
  ...
  PRODUCTOS_CATALOGO[idx] = nuevoNombre;
...
window.agregarProductoCatalogo = async function () {
  const input = document.getElementById('catalogo-nuevo-nombre');
  const nombre = input?.value?.trim();
  if (!nombre) {
    showToast('Escribe el nombre del producto', 'error');
    return;
  }
  if (PRODUCTOS_CATALOGO.includes(nombre)) {
    showToast('Ese producto ya existe en el catálogo', 'error');
    return;
  }
  PRODUCTOS_CATALOGO.push(nombre);
  const res = await Catalogo.guardar(PRODUCTOS_CATALOGO);
  if (res.success) {
    input.value = '';
...
async function generarPDF() {
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media print { body { padding: 12px; } .seccion { page-break-inside: avoid; } }
  </style></head><body>
  ...
  ${seccionesHTML}
  <div class="pdf-footer">Jicmar CRM · Reporte generado automáticamente · ${hoy}</div>
  </body></html>`;
...
function generarSeccionVentas(pedidos) {
  ...
  const vendidos = pedidos.filter(p => ['pagado', 'parcial'].includes(p.estado));
  const consignasActivas = pedidos.filter(p => p.tipoVenta === 'consignacion' && p.estado === 'entregado');
  const mapa = {};
  ...
  vendidos.forEach(p => {
    ...
    prods.forEach(pr => {
      ...
      if (!mapa[pr.producto]) {
        mapa[pr.producto] = { ventaDirecta: 0, consigLiquidada: 0, devuelto: 0, precio: 0, total: 0 };
      }
      const cant = pr.cantidadEntregada ?? pr.cantidad ?? 0;
      const precio = pr.precioUnitario ?? 0;
      if (p.tipoVenta === 'consignacion') {
        mapa[pr.producto].consigLiquidada += cant;
        mapa[pr.producto].devuelto += p.unidadesDevueltas || 0;
      } else {
  ...
  Object.values(mapa).forEach(v => {
    if (v.devuelto > 0) v.total -= v.devuelto * v.precio;
  });

  const filas = Object.entries(mapa).sort((a, b) => (b[1].ventaDirecta + b[1].consigLiquidada) - (a[1].ventaDirecta + a[1].consigLiquidada));
  ...
  const cobrado        = pedidos.filter(p => p.estado === 'pagado').reduce((s, p) => s + (p.montoCobrado || p.totalEntregado || p.total || 0), 0);
  const parcialCobrado = pedidos.filter(p => p.estado === 'parcial').reduce((s, p) => s + (p.montoCobrado || 0), 0);
  const totalCobrado   = cobrado + parcialCobrado;
  const totalEnCalle   = consignasActivas.reduce((s, p) => { const e = p.totalEntregado || p.total || 0; return s + (e - (p.montoCobrado || 0)); }, 0);

  let html = `
  <div class="seccion">
    <div class="seccion-titulo">📊 Ventas por producto</div>
    <div class="seccion-subtitulo">Venta directa + consignaciones liquidadas · ${vendidos.length} pedido(s) cerrado(s) en el período</div>
  ...
  if (filas.length === 0) {
    ...
    html += `<table><thead><tr>
      ...
      <th class="col-num">V. Directa</th><th class="col-num">Consig. Liquidada</th>
      <th class="col-num">Devuelto</th><th class="col-num">Total Vendido</th>
      <th class="col-num">P. Unit.</th><th class="col-num">Importe</th>
    </tr></thead><tbody>`;
    filas.forEach(([nombre, v], i) => {
      const totalPiezas = v.ventaDirecta + v.consigLiquidada - v.devuelto;
      html += `<tr>
        <td class="col-center">${i+1}</td><td>${nombre}</td>
        <td class="col-num">${v.ventaDirecta}</td><td class="col-num">${v.consigLiquidada}</td>
        <td class="col-num" style="color:${v.devuelto > 0 ? '#e65100' : '#aaa'};">${v.devuelto > 0 ? v.devuelto : '—'}</td>
    ...
    html += `</tbody><tfoot><tr class="fila-total">
      ...
      <td class="col-num">${totalDirecta}</td><td class="col-num">${totalConsigLiq}</td>
      <td class="col-num">${totalDevuelto}</td><td class="col-num">${totalVendido}</td>
      <td></td><td class="col-num">${fmt(totalImporte)}</td>
    </tr></tfoot></table>`;
  }

  html += `
    <div style="margin-top:20px;border:1px solid #e3f2fd;border-radius:8px;overflow:hidden;">
      <div style="background:#1565c0;color:#fff;padding:8px 14px;font-size:13px;font-weight:700;">📦 En consigna activa — stock en calle</div>`;
  ...
  if (consignasActivas.length === 0) {
    ...
    html += `<table><thead><tr>
      ...
      <th class="col-num">Entregado</th><th class="col-num">Ya cobrado</th><th class="col-num">Pendiente</th>
    </tr></thead><tbody>`;
    consignasActivas.forEach(p => {
      const entregado = p.totalEntregado || p.total || 0;
      const yaCobrado = p.montoCobrado || 0;
      const pendiente = entregado - yaCobrado;
      const prods = (p.productosEntregados || p.productos || []).map(pr => `${pr.producto} ×${pr.cantidadEntregada ?? pr.cantidad}`).join(', ');
      html += `<tr>
...
function generarSeccionClientes(pedidos) {
  const relevantes = pedidos.filter(p => ['pagado','parcial','entregado'].includes(p.estado));
  const mapaClientes = {};
  relevantes.forEach(p => {
    const id = p.clienteId || p.clienteNombre || 'desconocido';
    if (!mapaClientes[id]) mapaClientes[id] = { nombre: p.clienteNombre || '—', productos: {}, total: 0 };
    const prods = p.productosEntregados || p.productos || [];
    prods.forEach(pr => {
      if (!pr.producto) return;
      if (!mapaClientes[id].productos[pr.producto]) mapaClientes[id].productos[pr.producto] = { vendido: 0, precio: 0, total: 0 };
      const cant = pr.cantidadEntregada ?? pr.cantidad ?? 0;
  ...
  clientesOrdenados.forEach((cliente, idx) => {
    const prodOrdenados = Object.entries(cliente.productos).sort((a,b) => b[1].vendido - a[1].vendido);
    html += `<div class="cliente-bloque">
      <div class="cliente-bloque-header">
        <div><span style="color:#888;font-size:10px;margin-right:6px;">#${idx+1}</span>
          <span class="cliente-nombre">${cliente.nombre}</span></div>
        <span class="cliente-total-badge">${fmt(cliente.total)}</span>
      </div>
      <table><thead><tr><th>#</th><th>Producto</th>
        <th class="col-num">Cantidad</th><th class="col-num">P. Unit.</th><th class="col-num">Total</th>
      </tr></thead><tbody>`;
    prodOrdenados.forEach(([nombre, v], i) => {
      html += `<tr><td class="col-center">${i+1}</td><td>${nombre}</td>
        <td class="col-num">${v.vendido}</td><td class="col-num">${fmt(v.precio)}</td>
        <td class="col-num"><strong>${fmt(v.total)}</strong></td></tr>`;
    });
...
function generarSeccionConsignas(pedidos) {
  const enCalle = pedidos.filter(p => p.tipoVenta === 'consignacion' && ['entregado','parcial'].includes(p.estado));
  const fmt = v => v.toLocaleString('es-MX', { style:'currency', currency:'MXN' });
  ...
  let html = `<div class="seccion">
    ...
    <table><thead><tr>
      <th>Cliente</th><th>Productos</th>
      <th class="col-num">Entregado</th><th class="col-num">Cobrado</th>
      <th class="col-num">Pendiente</th><th>Estado</th>
    </tr></thead><tbody>`;
...
function generarSeccionInactivos() {
  ...
  const clientesActivos = new Set(App.pedidosList.filter(p => p.createdAt instanceof Date && p.createdAt > hace30).map(p => p.clienteId));
  ...
  let html = `<div class="seccion">
    ...
    <table><thead><tr><th>#</th><th>Cliente</th><th>Teléfono</th><th>Dirección</th></tr></thead><tbody>`;

  inactivos.forEach((c, i) => {
    html += `<tr><td class="col-center">${i+1}</td><td>${c.nombre || c.escuela || '—'}</td>
      <td>${c.telefono || '—'}</td><td style="font-size:10px;color:#888;">${c.direccion || '—'}</td></tr>`;
  });
  html += `</tbody></table></div>`;
  return html;
}
