...
function expandirProductoParaInventario(nombreProducto, cantidadPiezas) {
  if (esPaqueteMix(nombreProducto)) {
    const numPaquetes = cantidadPiezas / PIEZAS_POR_PAQUETE;
...
const PAQUETES = {
  ...
  'Paquete Chicharrón Mix': {
    categoria: 'Chicharrón',
    contenido: {
      'Chicharrón Sal y limón': 4,
      'Chicharrón Salsas negras': 4,
      'Chicharrón Flaming hot': 4
    }
  },
};
...
function calcularPedidoMayoreo({ nombreProducto, cantidad, unidad, nivel, modo }) {
  ...
  const piezasBase = unidad === 'paquetes' ? cantidad * PIEZAS_POR_PAQUETE : cantidad;
  ...
  if (modo === 'manual' && nivelUsado !== 'normal' && piezasBase > 0) {
    ...
    if (piezasRegalo > 0) {
      const paquetesRegalo = Math.floor(piezasRegalo / PIEZAS_POR_PAQUETE);
      const piezasRegaloSueltas = piezasRegalo % PIEZAS_POR_PAQUETE;
      sugerencia = {
        piezasRegalo,
        paquetesRegalo,
        piezasRegaloSueltas,
  ...
  return `
    <div class="form-row">
      <div class="form-group" style="margin-bottom:0.4rem;">
        ...
        ${esMix
          ? `<div style="padding:0.5rem;font-size:0.85rem;color:var(--text-muted);">Paquetes (12 pzas) — Mix</div>`
          : `<select onchange="actualizarProductoMayoreo(${idx},'unidadCaptura',this.value)">
              ...
              <option value="paquetes" ${unidad === 'paquetes' ? 'selected' : ''}>Paquetes (12 pzas)</option>
            </select>`
        }
      </div>
      <div class="form-group" style="margin-bottom:0.4rem;">
